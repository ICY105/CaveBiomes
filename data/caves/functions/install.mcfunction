...
#check DU
execute unless score $du.ver load matches 2010200.. run scoreboard players set $install cave_data 0
execute unless score $du.ver load matches 2010200.. run tellraw @a [{"text":"Error: Cave Biomes requires Datapack Utilities version +2.1.2. You can download that [here].","color":"red","clickEvent":{"action":"open_url","value":"https://github.com/ImCoolYeah105/Datapack-Utilities/releases"}}]

#print install message
execute if score $install cave_data matches 1 run tellraw @a [{"text":"[Loaded Cave Biomes v1.2.0]","color":"dark_green"}]
